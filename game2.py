...
# Определение фигур тетриса (координаты ячеек для разных поворотов)
SHAPES = [
    [[1, 5, 9, 13], [4, 5, 6, 7]],        # I
    [[1, 2, 5, 6]],                        # O
    [[1, 4, 5, 6], [1, 5, 6, 9],          # T
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_UP:
                # Пробуем повернуть фигуру, если это возможно
                old_rotation = current_figure.rotation
                current_figure.rotate()
                if check_collision(current_figure, 0, 0):
                    current_figure.rotation = old_rotation  # отменяем поворот, если есть столкновение
